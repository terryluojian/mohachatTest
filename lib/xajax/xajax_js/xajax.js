...
var xajax={abort: function () {	r = this.getRequestObject(); if (this.readyStateChange(r)) { return true; } else { return false; } },callOptions:{method:'post'},
...
readyStateChange:function(r){try{ if(r.status==200){ infoObj.hide('systemMessagesPane'); if(xajaxConfig.debug)xajax.DebugMessage("Received:\n"+r.responseText);if(r.responseXML&&r.responseXML.documentElement)
...
else{ infoObj.unableToConnect(); this.completeResponse(); if(this.arrayContainsValue(this.responseErrorsForAlert,r.status)){var errorString="Error: the server returned the following HTTP status: "+r.status;errorString+="\nReceived:\n"+r.responseText;alert(errorString);}
...
completeResponse:function(r){document.body.style.cursor='default';if(xajaxConfig.statusMessages==true)window.status='Done';},
