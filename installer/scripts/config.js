...
function swapRegtype() {
	selReg = getRadioGroup('registration').value;

	if (selReg == 0) {
		$('rowForgotPasswordLink').style.display = 'none';
		$('rowRegisterLink').style.display = 'none';
	} else if (selReg == 1) {
		$('rowForgotPasswordLink').style.display = 'table-row';
		$('rowRegisterLink').style.display = 'table-row';
	}
